...
function parola_enqueue_visualization_assets() {
    ...
    wp_enqueue_script(
        ...
        '1.0.77', // v1.0.76 + v1.0.77 (x-axis label rotation)
		...
		// STABLE VERSIONS: "1.0.51", "1.0.55", "1.0.60", "1.0.69", "1.0.71", "1.0.72", "1.0.73", "1.0.74", "1.0.75", "1.0.76"
...
function custom_d3_enqueue_editor_script() {
	...
	// Load the same dependencies and renderer used on the frontend so
	// Gutenberg can display a real live chart preview.
	wp_enqueue_script(
		...
		edit: function ( props ) {
			...
			children.push(
				el( SelectControl, {
					...
					value: attributes.logoStyle || 'parola logo with text.png',
					options: [
						{ label: __( 'Logo with Text', 'custom-d3' ), value: 'parola logo with text.png' },
						{ label: __( 'Logo Only', 'custom-d3' ), value: 'parola logo only.png' },
						{ label: __( 'All White', 'custom-d3' ), value: 'parola logo all white.png' }
					],
					onChange: function ( value ) {
						setAttributes( {
							logoStyle: value
